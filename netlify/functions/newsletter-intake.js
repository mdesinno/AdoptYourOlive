// netlify/functions/newsletter-intake.js
...
async function brevoUpsertContact({ email, attributes = {}, listIdsToAdd = [] }) {
  if (!email) return;
  await brevo.post('/contacts', {
    ...
    updateEnabled: true
  }).catch(e => console.warn('Brevo upsert failed', e?.response?.data || e.message));
}
...
exports.handler = async (event) => {
  ...
  try {
    const { email, language = 'it', name = '' } = JSON.parse(event.body || '{}');
    if (!email) return { statusCode: 400, body: JSON.stringify({ error: 'Email mancante' }) };

    const now = new Date().toISOString();

    // 1) Iscritti newsletter (UPsert per evitare doppioni)
    const NEWS_HEADER = ['Data','Email','Lingua'];
    await upsertByEmail('Iscritti newsletter', NEWS_HEADER, email, (current) => ([
      current['Data'] || now,
      email,
      language
    ]));
    ...
    // Se esiste già "Data ultimo ordine" → è un cliente => mandalo su Newsletter
    // Se non è cliente → mandalo sul Funnel 5 giorni
    let goesToNewsletter = false;

    await upsertByEmail('Archivio contatti', ARCH_HEADER, email, (current) => {
      goesToNewsletter = Boolean(current['Data ultimo ordine']); // se aveva un ordine
      return [
        email,
    ...
    // 3) Brevo: lista in base a "cliente/non cliente"
    const LIST_TARGET = goesToNewsletter
      ? Number(process.env.BREVO_LIST_NEWSLETTER)
      : Number(process.env.BREVO_LIST_FUNNEL5GIORNI);

    await brevoUpsertContact({
      email,
      attributes: { NOME: name || '', LINGUA: language.toUpperCase() },
      listIdsToAdd: [LIST_TARGET].filter(Boolean)
    });

    return { statusCode: 200, body: JSON.stringify({ ok: true, goesTo: goesToNewsletter ? 'NEWSLETTER' : 'FUNNEL5' }) };
  } catch (e) {
    console.error('newsletter-intake ERR', e);
